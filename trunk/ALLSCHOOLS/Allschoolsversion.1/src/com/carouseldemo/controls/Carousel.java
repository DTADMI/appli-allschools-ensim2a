...
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener {
     ...
     */
    private static final int SCROLL_TO_FLING_UNCERTAINTY_TIMEOUT = 200;
	 ...
	 * The axe angle
	 */
    private float mTheta = (float)(12.0f*(Math.PI/180.0));	
	...
	void layout(int delta, boolean animate){
        ...
        int angleUnit = (int) (360.0f / count);

        int angleOffset = mSelectedPosition * angleUnit;
        for(int i = 0; i< getAdapter().getCount(); i++){
        	int angle = (int) (angleUnit * i - angleOffset);
        	if(angle < 0.0f)
        		angle = (int) (360.0f + angle);
    ...
    private void Calculate3DPosition(CarouselImageView child, float diameter, float angleOffset){
    	angleOffset = angleOffset * (float)(Math.PI/180.0f);
    	
    	
    	float x = -(float)(diameter/(2)*Math.sin(angleOffset));
    	float z = diameter/(3/2) * (1.0f - (float)Math.cos(angleOffset));
    	float y =  -getHeight()/(3) + (float) (z* Math.sin(mTheta));
    ...
    private int calculateTop(View child, boolean duringLayout) {
        ...
        case Gravity.CENTER_VERTICAL:
            ...
            childTop = mSpinnerPadding.top + (availableSpace /(3/2));
            break;
        case Gravity.BOTTOM:
    ...
    private int getCenterOfGallery() {
        return (getWidth() - Carousel.this.getPaddingLeft() - Carousel.this.getPaddingRight()) / 3 + 
    ...
    private void setUpChild(CarouselImageView child, int index, float angleOffset) {
        ...
        if(mInLayout)
        {
	        h = (getMeasuredHeight() - getPaddingBottom() - getPaddingTop())/(5/2);
	        w = 345; 
        }
        else
        {
